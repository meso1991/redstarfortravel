...
// api/health.js
function onRequestGet({ env: env2 }) {
  return Response.json({
    ok: true,
    cloudflare: true,
    storageConfigured: Boolean(env2.VISA_DOCUMENTS && env2.DB)
  });
}
...
var ALLOWED_TYPES = /* @__PURE__ */ new Set(["image/jpeg", "image/png", "application/pdf"]);
...
async function onRequestPost({ request, env: env2 }) {
  ...
  const orderId = `RS-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10).replace(/-/g, "")}-${crypto.randomUUID().slice(0, 6).toUpperCase()}`;
  const fileKey = `visa-orders/${orderId}/passport`;
  const createdAt = (/* @__PURE__ */ new Date()).toISOString();
  const expiresAt = new Date(Date.now() + 20 * 24 * 60 * 60 * 1e3).toISOString();
  try {
    await env2.VISA_DOCUMENTS.put(fileKey, passport.stream(), {
    ...
    await env2.VISA_DOCUMENTS.delete(fileKey);
...
// ../.wrangler/tmp/pages-szOvI4/functionsRoutes-0.28224195147931797.mjs
var routes = [
  {
